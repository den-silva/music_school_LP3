...
public class ProfessoresDao implements IcrudPadrao<Professores>{
	...
	public void insert(Professores entidade) throws SQLException {
		...
		try(PreparedStatement ps = con.prepareStatement("INSERT INTO tb_professores"
	...
	public void update(Professores entidade) throws SQLException {
		...
		try(PreparedStatement ps = con.prepareStatement("UPDATE tb_professores SET " 
																		+ "nome=?, "
																		+ "email=?, "
																		+ "senha=? "
																		+ "WHERE id_professor=?");)
		{
			...
			ps.setString(2, entidade.getEmail());
			ps.setString(3,entidade.getSenha());
			ps.setInt(4, entidade.getId_professor());
			ps.executeUpdate();
